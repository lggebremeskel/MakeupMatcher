...
def openfilename():
    ...
    filename = filedialog.askopenfilename(
        title='"Image', filetypes=[("PNG files", "*.PNG")])
    return filename
...
def analyzer(picture):
    ...
    **Returns**
    ...
    shade_number = (int(np.mean(shade_values_red)), int(
        np.mean(shade_values_green)), int(np.mean(shade_values_blue)))
    return shade_number
...
class user_inf(tk.Frame):
        ...
        **Returns**:
        ...
        try:
            ...
            if brand == 'Fenty':
                ...
                self.fenty_shade_result = Label(
                    self.master, text='Your recommended Fenty shade is, ' + fenty_shade, fg='green')
            ...
            if brand == 'Lancome':
                ...
                self.lancome_shade_result = Label(
                    self.master, text='Your recommended Lancome shade is, ' + lancome_shade, fg='blue')
            ...
            if brand == 'Dior':
                ...
                self.dior_shade_result = Label(
                    self.master, text='Your recommended Dior shade is, ' + dior_shade, fg='brown')
                self.dior_shade_result.place(x=100, y=400)
                return dior_shade
        except AttributeError:
            self.error_msg = Label(
                self.master, text='Error: Please upload a .PNG image first.', fg='red')
    ...
    def widgets(self):
        ...
        self.upload_request = Label(
            self.master, text='Please upload a close up picture of your hand \nClick "Example Button" to see example image', fg='black')
        self.upload_request.place(x=200, y=100)
        self.exp_img = Button(self.master, text='Example',
                              fg='red', command=lambda: self.example_img())
        self.exp_img.place(x=450, y=110)
        self.up_img = Button(self.master, text='Upload Image',
                             command=lambda: self.image_opener())
        self.up_img.place(x=340, y=150)
        self.fenty = Button(self.master, text='Fenty', fg='blue',
                            command=lambda: self.compare(self.picture, 'Fenty'))
        self.fenty.place(x=200, y=250)
        self.lancome = Button(self.master, text='Lancome', fg='blue',
                              command=lambda: self.compare(self.picture, 'Lancome'))
        self.lancome.place(x=350, y=250)
        self.dior = Button(self.master, text='Dior', fg='blue',
                           command=lambda: self.compare(self.picture, 'Dior'))
